  ...
  for (const prop of propsList.value || []) {
    ...
    if (
      ...
      try {
        ...
        console.error("[��ѯ] ��ѯ����ͼƬ���ɼ�¼ʧ��:", error);
      }
    }
  }
};
  ...
  for (let i = 0; i < maxAttempts; i++) {
    ...
    try {
      ...
      if (task.status === "failed") {
        const reason = task.error || task.message || "����ִ��ʧ��";
...
const generatePropImage = async (prop: any): Promise<boolean> => {
  ...
  try {
    ...
    ElMessage.error(error.message || "����ʧ��");
    return false;
  } finally {
    generatingPropImages.value[prop.id] = false;
  }
...
const handleUploadSuccess = async (response: any) => {
  try {
    ...
    if (!imageUrl && !localPath) {
      ElMessage.error("�ϴ�ʧ�ܣ�δ��ȡ��ͼƬ��ַ");
      return;
    }
    ...
    ElMessage.error(error.message || "�ϴ�ʧ��");
  }
};
